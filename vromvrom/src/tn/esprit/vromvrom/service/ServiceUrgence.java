...
public class ServiceUrgence  implements IService<Urgence>{
    ...
    public void createOneUser (Urgence urgence) throws SQLException {
        String req = "INSERT INTO urgence (id_trajet, localisation, description, statuts) " +
                " VALUES (?, ?, ?, ?)";
        //System.out.println(req);
        PreparedStatement st = cnx.prepareStatement(req);
        st.setInt(1, urgence.getTrajet().getId_trajet());
        st.setString(2, urgence.getLocalisation());
        st.setString(3, urgence.getDescription());
        st.setString(4, urgence.getStatuts());


        st.executeUpdate();
        System.out.println("Urgence client ajouté !");
    }
    ...
    public ObservableList<Urgence> selectAll() throws SQLException {
        ...
        */
        ...
        String req = "SELECT *" +
                     "FROM urgence" +
                     " LEFT JOIN trajet ON urgence.id_trajet = trajet.id_trajet" +
                     " LEFT JOIN voiture_urgence ON urgence.id_voiture = voiture_urgence.id_voiture";
